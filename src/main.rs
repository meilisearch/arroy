...
use arroy::{Euclidean, Manhattan, NodeCodec, Reader, Writer, BEU32};
...
fn main() -> heed::Result<()> {
    ...
    let writer = Writer::<Manhattan>::prepare(&mut wtxn, dimensions, database)?;
    ...
    for result in database.remap_data_type::<LazyDecode<NodeCodec<Manhattan>>>().iter(&wtxn)? {
    ...
    let rtxn = env.read_txn()?;
    let reader = Reader::<Manhattan>::open(&rtxn, database, dimensions)?;
